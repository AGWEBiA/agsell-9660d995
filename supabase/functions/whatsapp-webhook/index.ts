...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const supabaseServiceKey = Deno.env.get("TARGET_SUPABASE_SERVICE_ROLE_KEY") || Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
      ...
      if (integration) {
        ...
        if (userId) {
          ...
          if (senderPhone && !isStatusOnly && !isBroadcast) {
            ...
            // ────────────────────────────────────────────────────────────────
            // GLOBAL AUTOMATION ENGINE (Phase 4)
            // ────────────────────────────────────────────────────────────────
            // Previously, we only searched for automations in the instance's org.
            // Now we search in ALL organizations where this phone is a registered contact.
