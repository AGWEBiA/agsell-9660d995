...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (defaultGateway === "kiwify" && settings.kiwify_enabled) {
      ...
      if (!checkoutUrlStr) {
        return new Response(
          JSON.stringify({ error: "Kiwify checkout URL not configured for this package. Contact admin." }),
      ...
      await supabase.from("communication_transactions").insert({
        ...
        description: `Compra pendente: ${pkg.name} (${pkg.credits} créditos)`,
      });

      console.log(`[COMM-PURCHASE] Kiwify checkout for ${user.email}, package ${pkg.name}`);
        ...
        : [{
            price_data: {
              ...
              product_data: {
                ...
                description: `${pkg.credits} créditos de comunicação (SMS + VoIP)`,
              },
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        mode: "payment",
        success_url: `${origin}/sms-marketing?purchase=success&package=${pkg.id}`,
        cancel_url: `${origin}/sms-marketing?purchase=cancelled`,
        metadata: {
      ...
      console.log(`[COMM-PURCHASE] Stripe checkout session ${session.id} for ${user.email}`);
    ...
    console.error("[COMM-PURCHASE] Error:", error);
