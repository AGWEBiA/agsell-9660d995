...
async function getDomainStatusOnResend(apiKey: string, domainId: string): Promise<{ status: string | null; records: any[] }> {
  try {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // --- Step 1: Register on Resend (single domain, NO inbound subdomain to save slots) ---
    ...
    if (resendApiKey) {
      ...
      if (providerDomainId) {
        ...
        const domainDetails = await getDomainStatusOnResend(resendApiKey, providerDomainId);
        resendStatus = domainDetails.status;
        // Always get latest records from Resend
        if (domainDetails.records.length > 0) {
    ...
    // Save real DNS records from Resend
    if (resendRecords.length > 0) {
      updateData.dns_records = resendRecords.map((r: any) => ({
        type: r.type || 'TXT',
      ...
      console.log(`Saved ${resendRecords.length} DNS records for domain ${domain}`);
    }
