...
import {
  ArrowRight, Play, Check, Star, Zap, MessageSquare, Mail, Bot,
  BarChart3, Users, Workflow, Phone, Instagram, Send, Brain,
  Target, Inbox, Globe, Shield, Sparkles, TrendingUp,
} from 'lucide-react';
...
const TRAILS = [
  ...
  {
    title: 'Trilha Comunicação Omnichannel',
  ...
  {
    title: 'Trilha Automação Avançada',
...
export default function Vendas() {
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white selection:bg-primary/30 antialiased">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#0A0A0A]/85 backdrop-blur-xl border-b border-white/5">
        <div className="container mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm text-white/70">
            ...
            <a href="#stack" className="hover:text-white transition-colors">Funcionalidades</a>
            <Link to="/pricing" className="hover:text-white transition-colors">Planos</Link>
            <Link to="/features" className="hover:text-white transition-colors">Recursos</Link>
          </nav>
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          <div className="grid lg:grid-cols-[1.1fr_1fr] gap-12 lg:gap-16 items-center max-w-6xl mx-auto">
            ...
            <div className="relative">
              ...
              <div className="relative aspect-video rounded-2xl border border-primary/20 bg-gradient-to-br from-zinc-900 to-zinc-950 overflow-hidden shadow-2xl shadow-primary/10">
                {/* fake browser chrome */}
                <div className="absolute top-0 left-0 right-0 h-8 bg-zinc-900/80 border-b border-white/5 flex items-center gap-1.5 px-3 z-10">
                  <div className="h-2.5 w-2.5 rounded-full bg-red-400/70" />
                  <div className="h-2.5 w-2.5 rounded-full bg-yellow-400/70" />
                  <div className="h-2.5 w-2.5 rounded-full bg-green-400/70" />
                  <div className="ml-3 px-2.5 py-0.5 rounded text-[10px] bg-white/5 text-white/40 font-mono">app.agsell.com.br</div>
                </div>
                {/* play button */}
                <button className="absolute inset-0 flex items-center justify-center group">
                  <div className="h-20 w-20 rounded-full bg-primary/90 backdrop-blur flex items-center justify-center group-hover:scale-110 transition-transform shadow-2xl shadow-primary/40">
                    <Play className="h-8 w-8 text-white ml-1" fill="currentColor" />
                  </div>
                </button>
                {/* gradient bg behind */}
                <div className="absolute inset-x-8 bottom-8 top-12 rounded-xl bg-gradient-to-br from-primary/20 via-purple-500/10 to-blue-500/20 flex items-end justify-center p-6">
                  <p className="text-xs text-white/50 font-mono">▶ Veja o AG Sell em ação</p>
                </div>
              </div>
              <p className="text-center text-xs text-white/30 mt-4 italic">2:34 — Tour completo da plataforma</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Trilhas (estilo Asimov) */}
      <section id="trilhas" className="py-20 md:py-28 border-t border-white/5">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="max-w-3xl mb-14">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 leading-tight">
              Aprenda por <span className="text-primary">trilhas</span> de funcionalidades
            </h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
            {TRAILS.map((t, i) => (
              <div
                key={i}
                className="group relative rounded-2xl border border-white/5 bg-gradient-to-br from-white/[0.03] to-transparent p-6 hover:border-primary/30 hover:from-primary/[0.04] transition-all cursor-pointer"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className={`h-11 w-11 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center`}>
                    <t.icon className={`h-5 w-5 ${t.color}`} />
                  </div>
                  <span className="text-[10px] uppercase tracking-wider text-white/30 font-semibold">{t.hours}</span>
                </div>
                <h3 className="text-lg font-bold mb-2 group-hover:text-primary transition-colors">{t.title}</h3>
