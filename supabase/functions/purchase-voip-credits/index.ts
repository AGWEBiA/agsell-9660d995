...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (defaultGateway === "kiwify" && settings.kiwify_enabled) {
      ...
      console.log(`[VOIP-PURCHASE] Kiwify checkout for ${user.email}, package ${pkg.name}`);
      ...
      // Use existing price or create ad-hoc line item
      const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] = pkg.stripe_price_id
        ...
        : [{
            price_data: {
              ...
              product_data: {
                name: `VoIP - ${pkg.name}`,
                description: `${pkg.credits} créditos de voz`,
              },
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        mode: "payment",
        success_url: `${origin}/voip?purchase=success&package=${pkg.id}`,
        cancel_url: `${origin}/voip?purchase=cancelled`,
        metadata: {
          type: "voip_credits",
          package_id: pkg.id,
          organization_id: membership.organization_id,
      ...
      console.log(`[VOIP-PURCHASE] Stripe checkout session ${session.id} for ${user.email}`);
    ...
    console.error("[VOIP-PURCHASE] Error:", error);
